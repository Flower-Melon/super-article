...
This section evaluates CLoMA-UWR with the evaluation suite in
Section~\ref{sec:evaluation-suite} through three settings. The first examines
whether CLoMA-UWR outperforms an Optimization-Based Planner as
...
and action space with CLoMA-UWR, and re-optimizes the remaining
...
Using the DeepSeek~V4~Pro configuration, we compare CLoMA-UWR with w/o
...
We construct 2,000 mission-planning demonstrations using CLoMA-UWR
...
range, the advantage of CLoMA-UWR also reflects task completion
...
higher operational pressure of the hard and expert episodes, CLoMA-UWR
...
ablated variant relative to CLoMA-UWR.

\begin{figure}[!t]
    \centering
    \includegraphics[width=\columnwidth]{figures/ablation-score-change-heatmap.png}
    \caption{Change in overall score relative to CLoMA-UWR for each ablated
...
effects explain why all three are retained in CLoMA-UWR despite
