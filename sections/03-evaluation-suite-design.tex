...
AirSim for examining initial mission planning and runtime plan repair under calibrated resource
bounds, perturbation conditions, and mission-level metrics. It instantiates the
task model in Section~\ref{sec:system-design} through real-to-sim calibration,
difficulty stratification, and mission-level metrics, which ground the simulated
setting, vary operational pressure, and distinguish task completion, execution
quality, and mission continuity. As summarized in
Table~\ref{tab:benchmark_comparison}, existing settings cover different parts of
the transition from aerial observation to task-zone construction,
resource-constrained mission planning, and runtime plan repair. The comparison
concerns evaluation coverage under their respective task definitions rather
than algorithmic performance. The following subsections detail the three design
elements.
...
engineering boundary of the response mission, while AirSim configures controllable
...
performance. Let $\mathcal{Z}$ be the set of task zones, $c_z\in[0,1]$ the completion
ratio of zone $z$, and $\omega_z$ its priority weight. Let
$\bar t_{\mathrm{resp}}$ and $T_{\mathrm{miss}}$ denote the priority-weighted mean time to
first effective intervention and the total mission duration. The six scores are
defined jointly as
\begin{equation}
\begin{aligned}
s_{\mathrm{comp}}
&=100\frac{\sum_{z\in \mathcal{Z}}\omega_zc_z}
 {\sum_{z\in \mathcal{Z}}\omega_z},\\
s_{\mathrm{resp}}
&=100\left[1-
...
s_{\mathrm{time}}
&=100\left[1-
\left[\frac{T_{\mathrm{miss}}-l_{\mathrm{time}}}
 {u_{\mathrm{time}}-l_{\mathrm{time}}}\right]_0^1\right],\\
s_{\mathrm{stab}}
&=\frac{100}{N_{\mathrm{evt}}}\sum_{a=1}^{N_{\mathrm{evt}}}
 \mathbb{I}\bigl(\mathrm{Recovered}(a)\bigr),\\
s_{\mathrm{res}}
 &=100\left[1-\frac{\bar p+\max_jp_j}{2}\right],\\
s_{\mathrm{coord}}
&=100\left[1-
\frac{G(\mathbf{n})+G(\boldsymbol{\ell})+G(\boldsymbol{\tau})}{3}\right].
\end{aligned}
...
$u_{\mathrm{time}}$ are fixed time-reference thresholds. $N_{\mathrm{evt}}$ is the number
of runtime perturbations, $a$ indexes these perturbations, and
$\mathrm{Recovered}(a)$ indicates recovery from perturbation $a$ by a feasible
...
counts $\mathbf{n}$, flight distances $\boldsymbol{\ell}$, or execution durations
$\boldsymbol{\tau}$.
...
For each episode $i$ at difficulty level $d$, the score is the weighted
aggregation of its applicable metrics. The reported score for level $d$ is the
mean across its $N_d$ episodes:
\begin{equation}
\begin{aligned}
S_d
...
is its normalized weight at difficulty level $d$. Because the component scores
lie within $[0,100]$ and the weights sum to one, $S_d$ also lies within $[0,100]$.
...
basis for testing the three questions examined next: whether CLoMC-UWR sustains
high-level closed-loop mission coordination, which mechanisms
